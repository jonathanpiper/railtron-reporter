...
def on_message(client, userdata, msg):
    ...
    if list(msgJSON)[0] == 'targetSensor' and (msgJSON['targetSensor'] == 'all' or msgJSON['targetSensor'] == hostname):
        ...
        if msgJSON['requestData'] == 'all':
            ...
            try:
                myriadDB = myriadAmp.get_current_ambientDB()
            except:
                myriadDB = 0
                print("Unable to retrieve ambient dB from Myriad amplifier.")
            try:
                myriad_settings = myriadAmp.get_current_myriad_settings()
            except:
                myriad_settings = {}
                print("Unable to retrieve settings from Myriad amplifier.")
            print('Temperature: ' + fg.li_red + str(temperature) + fg.rs + ' | ' + 'Humidity: ' +
                  fg.li_blue + str(humidity) + fg.rs + ' | ' + 'Ambient dB: ' + fg.li_green + str(myriadDB) + fg.rs)
            payload += '"sensor":"' + hostname + '","temperature":' + str(temperature) + ',"humidity":' + str(
                humidity) + ',"ambientDB":' + str(myriadDB) + ',"volumeOffset":' + myriad_settings.get('vol_offset', '0')
            ...
            payload += '"sensor":"'+hostname+'","volumeOffset":"' + \
                myriad_settings.get('vol_offset', '0')+'"'
        elif msgJSON['requestData'] == 'adjustVolumeOffset':
            try:
                myriad_settings = myriadAmp.update_myriad_volume_offset(msgJSON['parameter'])
            except:
                myriad_settings = {}
            #    print(
            #        "Unable to retrieve settings from Myriad amplifier volume offset.\n")
            payload += '"sensor":"'+hostname+'","volumeOffset":"' + \
                myriad_settings.get('vol_offset', '0')+'"'
